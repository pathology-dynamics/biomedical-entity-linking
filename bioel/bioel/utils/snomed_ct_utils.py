...
_SNOMED_DESC_FIELD_ACTIVE = 2  # description activity (deprecated description will have active = 0)
...
_SNOMED_DEF_FIELD_ACTIVE = 2  # definition activity (deprecated definition will have active = 0)
_SNOMED_DEF_FIELD_ID = 4  # concept id
_SNOMED_DEF_FIELD_DEF = 7  # term definition
...
class Snomed:
    ...
    def load_snomed(self):
        ...
        with open(os.path.join(
                self.snomed_path,
                f'Snapshot/Terminology/sct2_TextDefinition_Snapshot-en_INT_{self.release_id}.txt'
        ),
                  mode='r',
                  encoding='utf8') as f:
            ...
            for line in f:
                ...
                cid = definition[_SNOMED_DEF_FIELD_ID]  # concept id
                cdesc = definition[_SNOMED_DEF_FIELD_DEF]  # concept definition
                self.index_definition[cid] = cdesc    

        # load descriptions (aliases)
        with open(os.path.join(
                self.snomed_path,
            ...
            for line in f:
                ...
                if cid in nodes and (int(definition[_SNOMED_DESC_FIELD_ACTIVE])
                    ...
                    self.concept_index[cdesc] = cid
                    if cid not in self.index_concept:
                        self.index_concept[cid] = [cdesc]
                    else:
                        self.index_concept[cid].append(cdesc)
                # fi
            # for
        # with
